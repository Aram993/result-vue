<template>
      <span class="todo-list__item-text">{{ task.text }}</span>
      <button class="btn btn--check" aria-label="Завершить" :disabled="task.completed" @click="$emit('completeTask')">
      ...
      <button class="btn btn--delete" aria-label="Удалить" @click="$emit('removeTask')">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="15" height="22">
                <path
...
<script setup>
  defineProps(["task"]);
  defineEmits(["completeTask", "removeTask"]);
</script>
<style>
